...
for (const [width,name] of [[1440,'desktop'],[390,'mobile']]) {
  ...
  await page.locator('#faq-location').screenshot({path:resolve(root,'_previews',`location-faq-finish-${name}.png`)});
  await page.locator('#livraison').screenshot({path:resolve(root,'_previews',`location-delivery-finish-${name}.png`)});
  await page.close();
}
await browser.close();
console.log('captures finitions FAQ/livraison desktop + mobile générées');